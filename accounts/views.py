...
from phonenumbers import parse, is_valid_number, PhoneNumberFormat, format_number, NumberParseException
    ...
    def post(self, request):
        if 'phone_number' in request.POST:
            ...
            try:
                ...
                phone_number_int = int(format_number(parsed_number, PhoneNumberFormat.E164).replace('+', ''))
                verification_code = str(random.randint(1000, 9999))
                cache_key = f'sms_code_{phone_number_int}'
                cache.set(cache_key, verification_code, timeout=300)

                response = send_sms(phone=phone_number_int, message=f"Ваш код подтверждения: {verification_code}")
                logger.debug(f"Отправка SMS с кодом {verification_code} на номер {phone_number_int}")

                request.session['phone_number'] = phone_number_int
                messages.success(request, 'Код подтверждения отправлен на ваш телефон.')
                return render(request, 'accounts/login.html', {"code_sent": True})
                ...
                messages.error(request, "Произошла ошибка при отправке SMS. Пожалуйста, попробуйте позже.")
                return render(request, 'accounts/login.html', {"code_sent": False})
            ...
            logger.debug(f"Введенный код: {code}, Ожидаемый код: {expected_code} для номера: {phone_number}")
...
class ActivateInviteCodeAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not invite_code:
            return Response({"detail": "Инвайт-код не предоставлен."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if user.invited_by:
            return Response({"detail": "Вы уже активировали инвайт-код."}, status=status.HTTP_400_BAD_REQUEST)

        if inviter == user:
            return Response({"detail": "Нельзя использовать свой собственный инвайт-код."},
